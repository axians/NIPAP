...
class AvpSearchTest(unittest.TestCase):
    ...
    def test_values_and_quoted_spaces(self):
        for text, value in [('avp.site=stockholm', 'stockholm'),
    ...
    def test_not_equals_and_boolean_combinations(self):
        success, query = self.parser.parse('avp.site!=stockholm AND avp.owner OR avp.site=uppsala')
        self.assertTrue(success)
        sql, values = self.backend._expand_prefix_query(query)
        self.assertIn('AND', sql)
        self.assertIn('OR', sql)
        self.assertIn('!=', sql)
        self.assertEqual(values, ['site', 'stockholm', 'owner', 'site', 'uppsala'])
    ...
    def test_invalid_operators_and_empty_keys(self):
        ...
        for operator, key, value in [('=', '', 'x'), ('=', 'site', None),
                                     ('avp_exists', 'site', False)]:
            with self.assertRaises(NipapInputError):
                self.backend._expand_prefix_query({'operator': operator, 'val1': 'avp.' + key, 'val2': value})
        with self.assertRaises(NipapNoSuchOperatorError):
            self.backend._expand_prefix_query({'operator': 'contains', 'val1': 'avp.site', 'val2': 'x'})
